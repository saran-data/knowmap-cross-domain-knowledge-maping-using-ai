...
def init_neo4j_connection():
    ...
    try:
        ...
        print("⚠️ Application will continue without Neo4j features")
        print("⚠️ To enable Neo4j, make sure Neo4j database is running")
        return None
...
# REBEL model with robust error handling
re_pipeline = None
try:
    use_gpu = torch.cuda.is_available()
    device = 0 if use_gpu else -1
    
    # Try to load REBEL model with comprehensive error handling
    ...
    try:
        ...
        print(f"✅ REBEL model loaded from cache (GPU: {use_gpu})")
    except Exception as offline_error:
        print(f"REBEL model not in cache: {offline_error}")
        
        # Try online download
        print("Attempting to download REBEL model...")
        os.environ['TRANSFORMERS_OFFLINE'] = '0'
        
        try:
            # Download with progress
            from huggingface_hub import snapshot_download
            print("Downloading REBEL model (this may take a few minutes)...")
            snapshot_download(
                repo_id="Babelscape/rebel-large",
                local_dir="./models/rebel-large",
            ...
            re_pipeline = pipeline(
                ...
                model="./models/rebel-large",
                device=device
            )
            print(f"✅ REBEL model downloaded and loaded (GPU: {use_gpu})")
            
        except Exception as download_error:
            print(f"❌ REBEL model download failed: {download_error}")
            print("⚠️ Continuing without REBEL model - using spaCy extraction only")
            re_pipeline = None
    ...
    print(f"❌ REBEL model initialization failed: {e}")
    print("⚠️ Continuing without REBEL model")
    re_pipeline = None
